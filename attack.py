import cupy as cp
import numpy as np
from scipy.stats import multivariate_normal
from traces_bin_reader.TracesBin import TracesBin
import time

import glob
data_dir = "fpga100t_duDFS_V_1.00_F_50.000MHz/"

# All files and directories ending with .dat and that don't begin with a dot:
op_files = glob.glob(data_dir + "*.dat")

covMatrix = cp.load("./data/covMatrix.npy")
meanMatrix = cp.load("./data/meanMatrix.npy")
POIs = cp.load("./data/POIs.npy")

sbox = np.array([
    ])

log = []
t = 0
attempts = np.zeros(256)
exTime = np.zeros(256)

for op_file in op_files:

    s_time = time.perf_counter()

    f = TracesBin(op_file)
    [traces, texts] = f.getAllTraces()
    key = f.getKey()
    print("Attack key: " + str(key[0]))
    log.append("Attack key: " + str(key[0]))

    tempSbox = [sbox[texts[i][0] ^ key[0]] for i in range(len(texts))]

    hw = [bin(x).count("1") for x in range(256)]

    P_k = cp.zeros(256)

    for j in range(len(traces)):
        # Grab key points and put them in a matrix
        a = [traces[j][POIs[i].get()] for i in range(len(POIs))]

        # Test each key
        for k in range(256):
            # Find HW coming out of sbox
            HW = hw[sbox[texts[j][0] ^ k]]
            # Find p_{k,j}
            rv = multivariate_normal(meanMatrix[HW].get(), covMatrix[HW].get(), allow_singular=True)
            p_kj = rv.pdf(a)

            # Add it to running total
            P_k[k] += cp.log(p_kj)

        # Print our top 5 results so far
        # Best match on the right
        log.append(P_k.argsort()[-5:])

        if P_k.argsort()[-1] == key[0]:
            log.append("Key " + str(key[0]) + " found in: " + str(j + 1) + " traces")
            attempts[t] = j + 1
            break

    e_time = time.perf_counter()
    exTime[t] = e_time - s_time
    log.append(str(e_time - s_time) + " seconds")
    log.append("")
    t += 1

log.insert(0, "AVG Execution Time: " + str(np.average(exTime)) + " seconds")
log.insert(0, "AVG attempts: " + str(np.average(attempts)))


with open(r'./out.txt', 'w') as fp:
    for item in log:
        fp.write("%s\n" % item)
