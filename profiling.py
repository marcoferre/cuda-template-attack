...
for op_file in op_files:
    ...
    for i in range(len(traces)):
        HW = tempHW[i]
        tempTracesHWPOIs[HW].append(traces[i].take(POIs))
...
for HW in range(9):
    if len(tempTracesHWPOIs[HW]) > 0:
        for i in range(numPOIs):
            for j in range(numPOIs):
                x = tempTracesHWPOIs[HW][:, i]
